...
You can download and install **HareTyper** for your respective platform from the **[builds](./builds)** folder.

### Windows
- Download the `.exe` file from the `builds` folder and run the installer to install the app on your Windows system.

### Linux (Debian-based)
- Download the `.deb` file from the `builds` folder.
- Install it using the following command:
  ```bash
  sudo dpkg -i haretyper-<version>.deb
  ```
- After installation, run this command to set the necessary permissions:
  ```bash
  sudo chmod 4755 /opt/HareTyper/chrome-sandbox
  ```
...
For any issues or inquiries, feel free to reach out to the creator:
